...
class DynamixelInterface(object):
    ...
    def read_packet(self):
        ...
        self._response_max_elapsed = max(self._response_max_elapsed, \
                                              elapsed_time)
        #print self._response_max_elapsed
        if byte != 0xFF:
...
class DynamixelNetwork (DynamixelInterface):
    ...
    def synchronize(self):
        ...
        for (ident, servo) in self._dynamixel_map.items():
            if servo.changed:
                if not self.stopped:
                    ...
                    if servo.torque_control_enable:
                        data.append(servo.goal_torque & 0xFF)
                        data.append(servo.goal_torque >> 8)
                    else:
        ...
        if count != 0:
            if servo.torque_control_enable:  
                self.sync_write(defs.REGISTER.GoalTorque,count,data)
            else:
