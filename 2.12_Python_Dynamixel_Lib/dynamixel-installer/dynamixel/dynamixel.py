...
Modified for Torque Control October 2013
by Daniel J. Gonzalez for the MIT 2.12 class
dgonz@mit.edu
...
class Dynamixel (object):
    ...
    def set_register_value( self, register, value ):
        ...
        """
        if register in [defs.REGISTER.GoalPosition, defs.REGISTER.MovingSpeed, defs.REGISTER.GoalTorque]:
    ...
    def read_all( self ):
        ...
        values = self._dyn_net.read_registers( self._id, 
                                               ...
                                               defs.REGISTER.GoalTorque )
        for i, reg in enumerate( regs ):
            self[ reg ] = values[i]
            
    def read_range( self, firstReg, lastReg ):
        regs = range(firstReg,lastReg+2,2)
        regs.sort()

        values = self._dyn_net.read_registers( self._id, firstReg,lastReg)
    ...
    def stop( self ):
        ...
        """
        if (self.torque_control_enable):
            self.goal_torque = self.current_load
        else:
    ...
    def _get_goal_torque( self ):
        """getter"""
        return self._get_register_value( defs.REGISTER.GoalTorque )
    def _set_goal_torque( self, value ):
        """ setter """                
        self.set_register_value( defs.REGISTER.GoalTorque, value )     
        
    goal_torque = property( _get_goal_torque, _set_goal_torque )
    ...
    def _get_torque_control_enable( self ):
        """getter"""        
        return (self._get_register_value(defs.REGISTER.TorqueControlEnable) != 0)
    def _set_torque_control_enable( self, value ):
        """ setter """
        self.set_register_value(defs.REGISTER.TorqueControlEnable, 1 if value else 0 )

    torque_control_enable = property( _get_torque_control_enable, _set_torque_control_enable )
